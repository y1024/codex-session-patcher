...
- 数据拆成三层：项目、广告位、投放。
- 同一个广告位同一时间只允许一条投放；结束时间等于下一条开始时间不算冲突。
- 投放状态由“启用状态 + 当前时间”计算，不手写 `已排期 / 投放中 / 已过期`。
- 后台输入和展示时间使用北京时间；数据库保存 ISO 时间。
- 租金只作为后台管理字段，不影响前台展示。
- 每个广告位提供建议宽高比和建议尺寸，上传图片时提示比例是否匹配，但不强制拦截。
...
- `投放中`：`enabled = true`，当前时间在 `start_at` 和 `end_at` 之间。
- `已过期`：`enabled = true`，当前时间晚于 `end_at`。
...
- 拖拽图片到预览区上传。
- 选择图片后立即本地预览。
- 如果图片比例和广告位建议比例差距较大，显示提醒，但仍允许保存。
...
- 图片超过大小限制时，提示压缩后再上传。
- 投放时间缺失或结束时间早于开始时间时，保存失败。
- 同一广告位时间重叠时，保存失败并提示冲突投放。
...
现有 `ad_slots` 表是上一版临时结构，实现时需要用新迁移兼容或替换到三层模型。公开接口保持不变。
